...
public class StatementBuilderImpl implements StatementBuilder {
    ...
    private void createStatementsType1() {
        classes.forEach(c1 -> {
            classes.forEach(c2 -> {
                classes.stream()
                        .filter(c3 -> !c1.equals(c3) && !c2.equals(c3))
                        .forEach(c3 -> {
                            generatedStatementsType1.add("( " + c1 + " and " + c2 + " ) SubClassOf: " + c3);
                        });
            });
        });
    }
    ...
    private void createStatementsType2() {
        classes.forEach(c1 -> {
            classes.stream().filter(c2 -> !c1.equals(c2)).forEach(c2 -> {
                objectProperties.forEach(o -> {
                    generatedStatementsType2.add(c1 + " SubClassOf: ( " + o + " some " + c2 + " )");
                });
            });
        });
    }
    ...
    private void createStatementsType3() {
        classes.forEach(c1 -> {
            classes.stream().filter(c2 -> !c1.equals(c2)).forEach(c2 -> {
                objectProperties.forEach(o -> {
                    generatedStatementsType3.add("( " + o + " some " + c1 + " ) SubClassOf: " + c2);
                });
            });
        });
    }
